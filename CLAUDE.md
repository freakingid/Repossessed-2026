...
src/
  ...
  barrels.js       // Barrel entity, fire ladder, damage intake, roll/kick physics, detonation + shrapnel (G.shrapnel lives HERE, not projectiles.js — B7), chain reactions.  EXISTS (SPEC-BARRELS P2: entity decoration, fireStateOf/damageBarrel, the real detonateBarrelsInRadius filled into BOTH enemies.js and abilities.js, shotsVsBarrels. P3: kickBarrel + updateBarrels roll integrator [2nd sanctioned moveBody exception], carry/kick FSM in player.js, meleeExchange carried-barrel chip via registerBarrelDamage seam. P4: resolveDetonations [collect-then-resolve in updateBarrels] + detonateBarrel [owner derive, spawnShrapnel, barrel:exploded emit, dropBarrelLight, splice OR detonate-in-hand via notifyCarriedBarrelDestroyed] + updateShrapnel [the §7.2.4 species — free wall/crate bounce, crate push, −1 health per damaging hit vs enemy/player/barrel/spawner, chain adoption, sweeps] + chain:reaction callout. COMPLETE). BOOT must `import "./barrels.js"` so the barrel factory decoration + dual seam registration run before frame 1.
  pickups.js       // Contact collection, Magnet, gem despawn, pickup value tables (SPEC-PICKUPS). EXISTS (P2: the three factory-decoration wraps only — food→heal, treasure→points, powerup→power, mirroring makeSpawner/makeBarrel; `key` intentionally left unwrapped, no value field. No `updatePickups` yet — P3 owes the Magnet-pull→gem-age→contact pass + `pickup:collected` emit). BOOT must `import "./pickups.js"` **after** `level-loader.js` so the factory overrides win (last-wins `registerEntityFactory`) — deferred integration debt, same as `abilities.js`/`barrels.js`.
