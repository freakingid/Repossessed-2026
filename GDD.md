...
> **Extracted to [`GDD-ENEMIES.md`](GDD-ENEMIES.md)** — split out per the ADD convention once this section stabilized (ahead of the subsystem-#4 build). Section numbering is unchanged: all `§6`, `§6.1`–`§6.4` references elsewhere in this GDD, the `SPEC-*` files, and `STATUS.md` resolve into that file. It covers the enemy roster (§6.1), the canonical stat table (§6.2), spawners (§6.3), and the pathfinding/navigation infrastructure (§6.4 — **BUILT**, subsystem #3).
